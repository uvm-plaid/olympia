...
class AcornClientAgent(AggregationClient):
    def round(self, round_number, message):
        if round_number == 1:
            ...
            key = self.GF.Random((self.s_len), seed=self.personal_seed)
            ...
            for j in current_ns:
                ...
                s_ij = self.GF.Random((self.s_len), seed=seed_ij)
            ...
            masked_value = self.encode(key=key, value=self.client_value)
...
class AcornServiceAgent(DropoutAggregationServer):
    ...
    def round(self, round_number, messages):
        ...
        if round_number == 2:
            ...
            sis = {c: self.GF.Random((self.s_len), seed=int(seed)) for c, seed in seeds.items()}
